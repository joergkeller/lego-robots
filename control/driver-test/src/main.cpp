...
void setup()
{
  ...
  myMotorDriver.inversionMode(LEFT_MOTOR, 0);  // don't invert motor
  myMotorDriver.inversionMode(RIGHT_MOTOR, 0); // don't invert motor
...
void loop()
{
  ...
  //  This moves both motors up, one faster than the other to drive a circle.
  //  It uses .setDrive( motorNum, direction, level ) to drive the motors.

  for (int reverse = 0; reverse < 2; reverse++) 
  {
    Serial.print("Smoothly move motors up to speed (drive level 0 to 255)...");
    for (int i = 0; i < 256; i++)
    {
      myMotorDriver.setDrive( LEFT_MOTOR, reverse, i);
      myMotorDriver.setDrive( RIGHT_MOTOR, reverse, 20 + (i / 2));
      delay(STEP_DURATION);
    }
    ...
    Serial.print("Smoothly move motors up to speed...");
    for (int i = 0; i < 256; i++)
    {
      myMotorDriver.setDrive( LEFT_MOTOR, reverse, 20 + (i / 2));
      myMotorDriver.setDrive( RIGHT_MOTOR, reverse, i);
      delay(STEP_DURATION);
    }
    Serial.println(" and back");
    for (int i = 255; i >= 0; i--)
